...
hotel_minified = HotelMinified(hotel_name="Hotel 1",
                               description="Hotel Description",
                               number_of_reviewers=10,
                               review_points=4.5,
                               booking_link="https://hotel.com")

hotel_extended = HotelExtended(hotel_name="Hotel 2",
                               description="Hotel Description",
...
def test_data_serializer():
    ...
    hotel_minified_json = json.dumps(hotel_minified.dict(),
                                     cls=PydanticJSONEncoder,
                                     indent=4)
    hotel_extended_json = json.dumps(hotel_extended.dict(),
                                     cls=PydanticJSONEncoder,
                                     indent=4)
    assert isinstance(hotel_extended_json, str) and isinstance(
        hotel_minified_json, str)
